...
;(function($) {
    $.fn.formset = function(opts)
    {
        var options = $.extend({}, $.fn.formset.defaults, opts),
            ...
            insertDeleteLink = function(row) {
                if(row.find('a.' + options.deleteCssClass).length == 0){
                    if (row.is('TR')) {
        ...
        $$.each(function(i) {
            ...
            if(options.replaceDeleteByLink){
                if (del.length) {
                ...
                if(!row.is('.'+ options.originFormsetClass)){
                    if (del.length) {
                        del.remove();
                    }
                    insertDeleteLink(row);
        ...
        if ($$.length) {
            ...
            if (options.formTemplate) {
                ...
                if(options.isDjangoFormEmpty){
                    var idRegex = new RegExp('__prefix__');
                    template.find('input,select,textarea,label').each(function() {
                        if ($(this).attr("for")) $(this).attr("for", $(this).attr("for").replace(idRegex, 2012));
                        if ($(this).attr('id')) $(this).attr('id', $(this).attr('id').replace(idRegex, 2012));
                        if ($(this).attr('name')) $(this).attr('name', $(this).attr('name').replace(idRegex, 2012));
                    });
            ...
            addButton.click(function() {
                ...
                if(options.addButton){
                    row.insertBefore($(options.addButton)).show();
                }else{
    ...
    $.fn.formset.defaults = {
        ...
        addButton: null,                 // The jQuery selector to specify a button add
        isDjangoFormEmpty: false,        // specify if formTemplate is a django empty form
        replaceDeleteByLink: true,       // if true, all checkbox are will replace by a link that hidden them
        originFormsetClass: 'is_origin', // CSS class indicates forms with instance that exist
    };
})(jQuery)
